...
func NewNginxController(nginxConfPath string, local bool, healthStatus bool) (*NginxController, error) {
    ngxc := NginxController{
        ...
        nginxUpstreamsPath: path.Join(nginxConfPath, "upstreams.d"),
...
func (nginx *NginxController) DeleteIngress(name string) {
    ...
    filename_upstream := nginx.getUpstreamNginxConfigFileName(name)
    ...
    glog.V(3).Infof("deleting %v", filename_upstream)
    if !nginx.local {
        if err := os.Remove(filename_upstream); err != nil {
            glog.Warningf("Failed to delete %v: %v", filename_upstream, err)
        }
    }
}
...
func (nginx *NginxController) AddOrUpdateIngress(name string, config IngressNginxConfig) {
    ...
    filename_upstream := nginx.getUpstreamNginxConfigFileName(name)
    nginx.templateIt(config, filename_upstream, "upstream.tmpl")
}
...
func (nginx *NginxController) getUpstreamNginxConfigFileName(name string) string {
    return path.Join(nginx.nginxUpstreamsPath, name+".conf")
}

func (nginx *NginxController) templateIt(config IngressNginxConfig, filename string, file_template string) {
    tmpl, err := template.New(file_template).ParseFiles(file_template)
    if err != nil {
        glog.Fatal("Failed to parse template file %v", file_template)
    }
